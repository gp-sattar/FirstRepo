package com.grampower.fieldforce.Databases;
...
public class DataBase extends SQLiteOpenHelper {
    ...
    public String sql_query_attendance="create table attendance (id integer primary key autoincrement, date varchar(8),attendanceType varchar(10),currentLocation varchar(50),currentTime varchar(40),selfie_name varchar(40), selfie_data blob, syncStatus int);";
    public String sql_query_tracking="create table tracking (id integer primary key autoincrement, date varchar(10),currentTime varchar(40),currentLocation varchar(70), syncStatus int);";
    ...
    public  void  insertTrackedLocation(DataBase ob,String date,String currentTime,String currentLocation,int syncStatus){
        SQLiteDatabase sq=ob.getWritableDatabase();
        ContentValues cv=new ContentValues();
        cv.put("date",date);
        cv.put("currentTime",currentTime);
        cv.put("currentLocation",currentLocation);
        cv.put("syncStatus",syncStatus);
        sq.insert("tracking", null, cv);
        Log.d("database operation", "attendance is inserted");
    }

    public Cursor getTrackedLocation(DataBase object){
        Cursor cr=null;
        try{ SQLiteDatabase SQ=object.getReadableDatabase();
            String sql_query="select * from tracking";
            cr = SQ.rawQuery(sql_query, null);
        }catch(SQLiteException e){
            e.printStackTrace();
        }
